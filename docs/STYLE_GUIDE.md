...
drawer/corner-tag chrome, `BattleStage.tsx`'s battle log toggle overlay,
`MonsterHpPanel.tsx` (compact single-row HUD since 2026-09-26 — it now also carries the trainer
name that used to be a separate corner tag; keep it one row so large curios' heads stay clear), `MapCanvas.tsx`'s in-canvas name tags, and the on-screen `FloatingJoystick.tsx`
