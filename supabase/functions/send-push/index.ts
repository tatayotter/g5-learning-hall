...
// built here). verify_jwt on this function requires a real bearer JWT, and
// we forward that same JWT into the Supabase client below instead of using
// the service role — so RLS on push_subscriptions (owner must match
// auth.uid()'s bridged app_user_id or parent id) is what actually stops a
// caller from ever reading/sending to someone else's subscriptions, not
