...
const GREETINGS = [
  "I know they're cute but don't squish your Curios too hard.",
  'Your Curios are doing great! Although they did look at my broom, look at me, and then throw it out the window.',
  'One of them took my shoe!',
  'Your Curios are getting more clever everyday.',
  "They won't sleep without a bedtime story.",
];

// Slides in from the right whenever the Curio Arena tab is opened, plays a
// random greeting voice line, and shows the matching line in a speech bubble.
export default function CurioExpertNpc() {
  const [visible, setVisible] = useState(true);
  const [lineIndex] = useState(() => Math.floor(Math.random() * GREETINGS.length));

  useEffect(() => {
    const audio = new Audio(`/sounds/voice/curioexpert_greeting_${lineIndex + 1}.mp3`);
    audio.volume = 0.7;
      ...
      {visible && (
        ...
        >
          <div className="relative bg-white text-black text-sm font-semibold rounded-2xl px-4 py-3 max-w-[220px] shadow-[3px_3px_0_0_#000] border-2 border-black">
            {GREETINGS[lineIndex]}
            <p className="mt-1 text-xs italic font-normal text-right text-gray-500">-Curio Expert</p>
            <div className="absolute top-1/2 -right-2 -translate-y-1/2 w-4 h-4 bg-white border-t-2 border-r-2 border-black rotate-45" />
          </div>
          <img
            src="/tap_npc_sprites/curio_expert.webp"
            alt="Curio Expert"
